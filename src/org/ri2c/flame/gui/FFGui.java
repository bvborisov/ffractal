...
public class FFGui extends JPanel {
	
	private static final long serialVersionUID = -2861824179795521368L;

	protected class FunctionPane extends JPanel implements ActionListener,
			FFWizard.FFWizardListener, FFWizardArray.FFWizardArrayListener {
		...
		public FunctionPane() {
			...
			addAFF = new JButton("new array of functions");

			addFF.setActionCommand("ff.add");
			addFF.addActionListener(this);

			addAFF.setActionCommand("ff.aa");
			addAFF.addActionListener(this);

			c.gridwidth = GridBagConstraints.REMAINDER;
			c.anchor = GridBagConstraints.WEST;
			bag.setConstraints(addFF, c);
			bag.setConstraints(addAFF, c);
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand().equals("ff.add")) {
				...
				ffw.setVisible(true);
			} else if (e.getActionCommand().equals("ff.aa")) {
				// System.out.println("add some funs");
				FFWizardArray ffwa = new FFWizardArray(frame);
				ffwa.addWizardArrayListener(this);
		...
		@Override
		public void ffWizardArrayFinished(FFWizardArray ffwa)
				throws InstantiationException, IllegalAccessException {

			FFunction[] ffs = ffwa.getFFunctions();
			int n = (Integer) ffwa.num.getValue();
			if ((ffs != null))
				for (int i = 0; i < n; i++)
					fflist.addFFunction(ffs[i]);

		}
	}

	protected class SpaceSelector extends JPanel implements ActionListener {
		/**
		 * 
		 */
		...
		public SpaceSelector() {
			xmin = new SpinnerNumberModel(new Double(-1), null, null,
					new Double(0.0001));
			xmax = new SpinnerNumberModel(new Double(1), null, null,
					new Double(0.0001));
			ymin = new SpinnerNumberModel(new Double(-1), null, null,
					new Double(0.0001));
			ymax = new SpinnerNumberModel(new Double(1), null, null,
					new Double(0.0001));
			xorg = new SpinnerNumberModel(new Double(0), null, null,
					new Double(0.0001));
			yorg = new SpinnerNumberModel(new Double(0), null, null,
					new Double(0.0001));
		...
		public Discretiser createDiscretiser(int width, int height) {
			return new Discretiser(xmin.getNumber().doubleValue(), ymin
					.getNumber().doubleValue(), xmax.getNumber().doubleValue(),
					ymax.getNumber().doubleValue(), width, height);
		}
		...
		protected void build() {
			...
			JLabel xminL = new JLabel("<html>x<sub>min</sub></html>"), xmaxL = new JLabel(
					"<html>x<sub>max</sub></html>"), xorgL = new JLabel(
					"<html>x<sub>org</sub></html>");
			JLabel yminL = new JLabel("<html>y<sub>min</sub></html>"), ymaxL = new JLabel(
					"<html>y<sub>max</sub></html>"), yorgL = new JLabel(
					"<html>y<sub>org</sub></html>");
		...
		public void computeBest() {
			Space s = Space.computeBestSpace(fPane.getFinalFFunction(),
					fflist.getFunctions());
	...
	protected class Output extends JPanel implements ActionListener {
	...
	protected class Rendering extends JPanel implements ActionListener {
		/**
		 * 
		 */
		private static final long serialVersionUID = -3011399608237796907L;

		DefaultListModel<RenderingAction> model;

		public Rendering() {
			model = new DefaultListModel<RenderingAction>();
		...
		protected void build() {
			...
			final JList<RenderingAction> list = new JList<RenderingAction>(model);
			...
			URL url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/rotate.png");
			ImageIcon icon;
			...
			url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/translate.png");

			if (url != null) {
				icon = new ImageIcon(url);
				translate = new JButton(icon);
			} else
				translate = new JButton("T");
			...
			url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/compute.png");

			if (url != null) {
				icon = new ImageIcon(url);
				computePoints = new JButton(icon);
			} else
				computePoints = new JButton("P");
			...
			url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/render-points.png");

			if (url != null) {
				icon = new ImageIcon(url);
				renderPoints = new JButton(icon);
			} else
				renderPoints = new JButton("RP");
			...
			url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/direct.png");

			if (url != null) {
				icon = new ImageIcon(url);
				direct = new JButton(icon);
			} else
				direct = new JButton("D");
			...
			url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/renderer.png");

			if (url != null) {
				icon = new ImageIcon(url);
				pushRenderer = new JButton(icon);
			} else
				pushRenderer = new JButton("SR");
			...
			url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/color-provider.png");

			if (url != null) {
				icon = new ImageIcon(url);
				pushColorProvider = new JButton(icon);
			} else
				pushColorProvider = new JButton("SCP");
		...
		public void actionPerformed(ActionEvent ae) {
			if ("rotation".equals(ae.getActionCommand())) {
				...
				addNewRenderingAction(RenderingAction
						.createGeneratePointsAction());
			} else if ("renderPoints".equals(ae.getActionCommand())) {
				addNewRenderingAction(RenderingAction
						.createRenderPointsAction());
	...
	protected class Compute extends JPanel implements ActionListener {
		/**
		 * 
		 */
		...
		protected void build() {
			...
			URL url = ClassLoader
					.getSystemResource("org/ri2c/flame/gui/compute-32.png");
			ImageIcon icon = null;
			...
			scrollPending.setBorder(BorderFactory
					.createTitledBorder("pending..."));
		...
		public void compute() {
			...
			rctx = new RenderingContext(ctx, output.getOutputFileName(),
					discretiser);
	...
	protected class ComputeThread extends JProgressBar implements Runnable {
		/**
		 * 
		 */
		...
		public ComputeThread(RenderingContext ctx,
				LinkedList<RenderingAction> actions) {
	...
	protected void build() {
		...
		rendering.setBorder(BorderFactory
				.createTitledBorder("rendering actions"));
